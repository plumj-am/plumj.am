...
use super::super::data::{ME, PROJECTS, VERSION};
use super::keyboard::KEYMAPS;
use super::use_keyboard_handler;
use super::use_scroll_snapping;
use dioxus::prelude::*;

fn get_nerd_file_info(route: &Route) -> Option<(String, String)> {
	// TODO: Remove after more routes have been added.
	#[allow(clippy::match_wildcard_for_single_variants)]
	match route {
		Route::NerdHome {} | Route::NerdPageNotFound { .. } => {
			Some(("portfolio/main.rs".to_string(), "rust".to_string()))
		}
		Route::NerdProject { name } => {
			let tech = PROJECTS
				.iter()
			...
			Some((format!("portfolio/{clean_name}.{ext}"), tech))
		}
		_ => None, // Not a nerd route.
	}
}
...
pub fn Layout() -> Element {
	use_scroll_snapping();
	rsx! {
		document::Stylesheet { href: asset!("/assets/gen-nerd-tailwind.css") }
		div {
...
pub fn Footer() -> Element {
	...
	if let Some((filename, filetype)) = get_nerd_file_info(&curr_route) {
		...
		// Not a nerd route, don't render footer.
		rsx! {}
	}
}
